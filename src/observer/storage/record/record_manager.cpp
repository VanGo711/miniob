...
RC PaxRecordPageHandler::get_chunk(Chunk &chunk)
{
  ...
  Bitmap bitmap(bitmap_, page_header_->record_capacity);
  int idx = bitmap.next_setted_bit(0);
  while(idx != -1){
    for(int i = 0 ;i<chunk.column_num();i++){
      int id = chunk.column_ids(i);
      Column* col = chunk.column_ptr(i);
      if(id ==0){
        col->append_one(frame_->data() + page_header_->data_offset + col->attr_len()*idx);
      }else{
        col->append_one(frame_->data() + page_header_->data_offset + col->attr_len()*idx + col_idx[id-1]);
      }
    }
